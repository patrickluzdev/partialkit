...
The components are `checkbox`, `contextMenu`, `dialog`, `dropdownMenu`, `popover`, `slider`,
`tabs`, `theme`, `toggle` and `tooltip`. Everything else is CSS only and needs no registration.
