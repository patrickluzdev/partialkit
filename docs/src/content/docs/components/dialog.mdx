...
`.dialog-close` positions a control in the corner and reserves room for it in the header. It is
plain markup, so leave it out when a dialog must be answered rather than dismissed. Give it an
`aria-label`, since the icon carries no text.
