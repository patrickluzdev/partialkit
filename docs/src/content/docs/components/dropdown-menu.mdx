...
`.dropdown-menu-shortcut` pushes a key hint to the trailing edge. Put the keys inside it as
[Kbd](/components/kbd/) elements — one per key — and add `kbd-group` so they sit together. It is a
...
| <kbd class="kbd">→</kbd> | Open the submenu under the cursor |
| <kbd class="kbd">←</kbd> | Close a submenu and return to its trigger |
| <kbd class="kbd">Esc</kbd> / <kbd class="kbd">Tab</kbd> | Close and return focus to the trigger |
